...
class Kiwoom(QAxWidget):
    ...
    def get_price_data(self, code):
        ...
        while self.has_next_tr_data:
            ...
            for key, val in self.tr_data.items():
                oh1cv[key] += val
        df = pd.DataFrame(oh1cv, columns=['open', 'high', 'low', 'close', 'volume'], index=oh1cv['date'])
    ...
    def _on_receive_tr_data(self, screen_no, rqname, trcode, record_name, next, unused1, unused2, unused3, unused4):
        ...
        if rqname == "opt10081_req":
            oh1cv = {'date': [], 'open': [], 'high': [], 'low': [], 'close': [], 'volume': []}
            ...
            deposit = self.dynamicCall("GetCommData(QString,QString,int,QString)",trcode,rqname,00,"주문가능금액")
            ...
            for i in range(tr_data_cnt):
                ...
                self.order[code]={
                    '종목코드':code, '종목명':code_name, '주문번호':order_number, '주문상태':order_status, '주문수량':order_quantity,
                    '주문가격':order_price, '현재가':current_price,'주문구분':order_type, '미체결수량':left_quantity, '체결량':executed_quantity,
                    '주문시간': ordered_at, '당일매매수수료':fee,'당일매매세금':tax}
            ...
            for i in range(tr_data_cnt):
                ...
                self.balance[code]={'종목명':code_name, '보유수량':quantity,'매입가':purchase_price,
                                    '수익률':return_rate, '현재가':current_price,'매입금액':total_purchase_price, '매매가능수량': available_quantity}
    ...
    def get_balance(self):
        ...
        self.dynamicCall("CommRqData(QString, QString, int, QString)", "opw00018_req", "opw000018", 0, "0002")
        self.tr_event_loop.exec_()
        return self.tr_data
    ...
    def _on_receive_real_data(self, s_code, real_type, real_data):
        if real_type == '장시작시간':
            ...
            self.universe_realtime_transaction_info[s_code].update({
                "체결시간": signed_at, "시가": open, "고가": high, "저가":low, "현재가":close,
                "(최우선)매도호가": top_priority_ask, "(최우선)매수호가": top_priority_bid,
                "누적거래량": accum_volume})